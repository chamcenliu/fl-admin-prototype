...
export const prdNotes: PrdNote[] = [
  ...
  },
  {
    ...
    priority: "P0",
    status: "可验收",
    description: "表格需要保留编号、模板标题、适用范围、状态和操作列，其中模板标题列仅展示模板名称，并支持新建、启停、推荐、预览、编辑、删除及批量修改适用范围。新建和编辑授权策略模板都属于核心条目的长表单操作，需要进入独立页面承载。",
    acceptance: ["表格展示编号、模板标题、适用范围、启停状态和操作，模板标题下不再重复展示编号", "行级操作支持启用/停用、推荐/取消推荐、预览、编辑和删除", "点击新模板进入独立新建页面，不使用弹窗承载长表单", "点击编辑进入独立编辑页面，不使用弹窗承载长表单", "新建页和编辑页都支持保存或取消后返回模板列表", "批量选择后出现“已选中 N 条”和“修改适用范围”操作"],
    linkedIterationId: "it-20260914-prd-shell"
  },
  {
    id: "prd-order-filter",
