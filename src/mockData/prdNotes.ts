...
export const prdNotes: PrdNote[] = [
  ...
  },
  {
    ...
    priority: "P0",
    status: "可验收",
    description: "表格需要保留 Axure 中的编号、授权策略模板、适用范围、状态和操作列，并支持启停、推荐、预览、编辑、删除及批量修改适用范围。编辑授权策略模板属于主列表条目的长表单编辑，需要进入独立页面承载。",
    acceptance: ["表格展示编号、模板名称、适用范围、启停状态和操作", "行级操作支持启用/停用、推荐/取消推荐、预览、编辑和删除", "点击编辑进入独立编辑页面，不使用弹窗承载长表单", "编辑页支持保存或取消后返回模板列表", "批量选择后出现“已选中 N 条”和“修改适用范围”操作"],
    linkedIterationId: "it-20260914-prd-shell"
  },
  {
    id: "prd-order-filter",
